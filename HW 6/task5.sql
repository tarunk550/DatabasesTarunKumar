...
SELECT 
    ...
    COUNT(restBill.bill_no) AS bill_no  -- Counting the number of bills per waiter per date
FROM restBill
JOIN restStaff ON restBill.waiter_no = restStaff.staff_no
GROUP BY restStaff.staff_no, restBill.bill_date  -- Group by waiter and date
HAVING 
    COUNT(restBill.bill_) >= 2;  -- Only include waiters with 2 or more bills on the same date
